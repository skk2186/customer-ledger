...
启动时会准备用户目录，检查 SQLite 完整性，并在已有账库升级前保存升级前备份；随后只通过版本化迁移升级，不调用 create_all。升级失败时程序停止启动并保留原账库和升级前备份。

如果恢复流程出现不可安全回滚的错误，程序会保留保护标记并阻止写入、导出等高风险操作。请停止记账，保留账库和备份，联系管理员处理。
