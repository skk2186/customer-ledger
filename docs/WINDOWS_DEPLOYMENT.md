...
| 导出设置 | %LOCALAPPDATA%\CustomerLedger\settings.json |

程序目录只保存可替换的应用文件。更新前应先关闭程序，保留用户数据目录和备份目录，再整体替换 CustomerLedger 程序目录。程序通过 Windows 命名互斥体阻止同一用户重复启动。

## 导出设置

普通用户可以在“导出设置”页面选择导出文件夹，也可以直接填写完整路径。Windows 桌面版提供原生文件夹选择器；取消选择不会改变当前设置。系统会创建并试写新文件夹，不可写时明确提示，不会静默回到其他目录。设置文件是 UTF-8 JSON，只保存导出目录，不保存客户或账务数据，也不能改变账库、备份或保护标记路径。

导出文件会直接写入设置的文件夹，成功页面显示中文文件名和保存位置，并可打开该文件夹。同一天重复导出会追加序号，不覆盖已有文件。
